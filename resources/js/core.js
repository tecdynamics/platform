...
class Tec {
    ...
    constructor() {
        ...
        Tec.initNavbarMinimal()
        Tec.initResources()
        Tec.initGlobalResources()
        Tec.handleCounterUp()
        Tec.initMediaIntegrate()

        if (
            TecVariables &&
            TecVariables.authorized === '0' &&
            typeof TecVariables.authorize_url !== 'undefined'
        ) {
    ...
    static blockUI(options) {
        ...
        Tec.showLoading(options.target)
    }

    static unblockUI(target) {
        Tec.hideLoading(target)
    }
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (Tec.noticesTimeout[key]) {
            clearTimeout(Tec.noticesTimeout[key])
        }

        Tec.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case 'error':
    ...
    static async copyToClipboard(textToCopy, parentTarget) {
        if (navigator.clipboard && window.isSecureContext) {
            ...
            Tec.unsecuredCopyToClipboard(textToCopy, parentTarget)
        }
    }
    ...
    static initResources() {
        ...
        Tec.initCodeEditorComponent()
        Tec.initColorPicker()
        Tec.initLightbox()
        Tec.initTreeCategoriesSelect()
        Tec.initCoreIcon()
    ...
    static initGlobalResources() {
        ...
        Tec.initTreeCheckboxes()
        Tec.initClipboard()
        Tec.initDropdownCheckboxes()
    }
    ...
    handleTurnOffDebugMode() {
        ...
        $submit.on('click', function (event) {
            ...
            Tec.showButtonLoading($submit[0])
            ...
            $httpClient
                ...
                .finally(() => {
                    Tec.hideButtonLoading($submit[0])
                })
        })
    }
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
            Tec.gallerySelectImageTemplate = `
    ...
    processAuthorize() {
        $httpClient
            .makeWithoutErrorHandler()
            .post(TecVariables.authorize_url)
            .catch(() => {})
    }
    ...
    countMenuItemNotifications() {
        ...
        if ($menuItems.length) {
            $httpClient
                .make()
                .get($menuItems.data('url') || TecVariables.menu_item_count_url)
                .then(({ data }) => {
                    data.data.map((x) => {
                        if (x.value > 0) {
    ...
    static initFieldCollapse() {
        $(document).on('click, change', '[data-bb-toggle="collapse"]', function (e) {
            ...
            switch (e.currentTarget.type) {
                ...
                default:
                    console.warn(`[Tec] Unknown type ${e.currentTarget.type} of collapse`)
    ...
    static initCodeEditorComponent() {
        $(document)
            ...
            .each(function () {
                ...
                Tec.initCodeEditor(this, this.dataset.mode || 'htmlmixed')
            })
    }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            if (!instance[instanceName]) {
                instance[instanceName] = Tec.lightbox()
            }
    ...
    static initClipboard() {
        $(document).on('click', '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            await Tec.copyToClipboard(text, clipboardParentTarget)

            if (copiedMessage) {
                Tec.showSuccess(copiedMessage)
            }
        ...
        $element.editable({
            ...
            success: function (response) {
                if (response.error && response.message) {
                    Tec.showError(response.message)
                }
            },
            error: function (response) {
                Tec.handleError(response)
            },
        })
    }
