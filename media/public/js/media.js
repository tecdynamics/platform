...
var MediaDetails = /*#__PURE__*/function () {
  ...
  _createClass(MediaDetails, [{
    ...
    value: function renderData(data) {
      var _this = this;
      var _self = this;
      var thumb = data.type === 'image' ? ' <img  loading="lazy" src="' + data.full_url + '" alt="' + data.name + '">' : '<i class="' + data.icon + '"></i>';
      var description = '';
    ...
    value: function renderData(data) {
      ...
      _.forEach(data.files, function (value) {
        ...
        if (_Helpers_Helpers__WEBPACK_IMPORTED_MODULE_0__.Helpers.getRequestParams().view_type === 'list') {
          ...
          item = item.replace(/__thumb__/gi, value.type === 'image' ? ' <img  loading="lazy" src="' + (value.thumb ? value.thumb : value.full_url) + '" alt="' + value.name + '">' : '<i class="' + value.icon + '"></i>');
        }
        var $item = $(item);
